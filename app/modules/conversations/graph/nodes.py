...
def _split_structured_checkout_lines(lines: list[str]) -> list[str] | None:
    ...
    if payment_index < 0:
        return None
    before_payment = useful_lines[:payment_index]
    after_payment = useful_lines[payment_index + 1 :]
...
def _split_rich_address_line(text: str) -> tuple[str, str | None, str | None]:
    ...
    before = re.sub(r"\b(?:barrio|sector)\s*$", "", raw[:start].strip(" ,.-"), flags=re.IGNORECASE).strip(" ,.-")
    after = raw[end:].strip(" ,.-")
