    ...
    for line in text.splitlines():
        ...
        if _looks_like_phone(line) or _looks_like_payment_method(normalized) or _looks_like_address(normalized):
            if (
                ...
                and re.search(r"\b(cra|cr|carrera|carrea|calle|cll|cl|sector|bloque|apto|apartamento)\s*\w+", normalized) is None
            ):
                continue
            return True
    return False
...
def _has_street_address_marker(normalized: str) -> bool:
    return bool(
        ...
        or re.search(
            r"\b(?:cra|cr|carrera|carrea|calle|cll|cl|avenida|avda|av|transversal|tv|diagonal|dg)\s*\d",
            normalized,
        )
    )
