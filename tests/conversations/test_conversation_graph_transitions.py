...
@pytest.mark.asyncio
async def test_pickup_checkout_accepts_standalone_time_when_waiting_for_pickup_time() -> None:
    services = FakeConversationServices()
    services.session.add_cart_item(cart_item_from_product(services.products["BROASTER_MEDIO"], 1))
    services.session.customer_name = "Cesar Mantilla"
    services.session.customer_phone = "3174121205"
    services.session.fulfillment_type = "PICKUP"
    services.session.move_to(ConversationState.ASK_CUSTOMER_DATA)
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="15 minutos"))

    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert services.session.observations == "Recoge en 15 minutos"
    assert "Revisa tu orden" in result["response_text"]


@pytest.mark.asyncio
async def test_checkout_accepts_standalone_customer_name_while_data_is_missing() -> None:
    services = FakeConversationServices()
    services.session.add_cart_item(cart_item_from_product(services.products["BROASTER_MEDIO"], 1))
    services.session.move_to(ConversationState.ASK_CUSTOMER_DATA)
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Saly peña Jaimes"))

    assert result["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert services.session.customer_name == "Saly peña Jaimes"
    assert "nombre completo" not in result["response_text"]
    assert "telefono" in result["response_text"]


@pytest.mark.asyncio
async def test_checkout_repeated_product_with_sauce_note_does_not_open_product_menu() -> None:
    services = FakeConversationServices()
    services.session.add_cart_item(cart_item_from_product(services.products["BROASTER_MEDIO"], 1))
    services.session.customer_name = "Saly peña Jaimes"
    services.session.customer_phone = "3182705144"
    services.session.customer_address = "Cr 31 # 155-30 piso 1"
    services.session.customer_neighborhood = "asovilagos"
    services.session.move_to(ConversationState.ASK_CUSTOMER_DATA)
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Medio pollo a la broaster con tártara y miel bastante acabé de llamar\n"
                "Para confirmar la dirección\n"
                "Cr 31 # 155-30 piso 1 asovilagos"
            ),
        )
    )

    assert result["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "Pollo broaster" not in result["response_text"]
    assert "metodo de pago" in result["response_text"]
    assert "tártara" in (services.session.observations or "")


@pytest.mark.asyncio
async def test_drink_menu_numbers_are_not_quantities_when_product_name_follows() -> None:
    parsed = parse_natural_order_rules(
        "1/4 Broaster 13500\n"
        "2.Jugo Hoy Persona 3000\n"
        "6.Coca-colq personal 400 ml"
    )
...
@pytest.mark.asyncio
async def test_then_restatement_replaces_existing_cart_instead_of_adding_duplicate() -> None:
    services = FakeConversationServices()
    services.products["ASADO_ENTERO"] = Product(
        code=ProductCode("ASADO_ENTERO"),
        name=ProductName("1 Asado Entero"),
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Entonces es tan amable un pollo asado y un cuarto pero pierna asado",
        )
    )

    assert result["current_step"] == ConversationState.POST_ADD
    assert [(item.product_code.value, item.quantity, item.product_name.value) for item in services.session.cart] == [
        ("ASADO_ENTERO", 1, "1 Asado Entero"),
        ("ASADO_CUARTO", 1, "1/4 Asado - Pierna"),
    ]


@pytest.mark.asyncio
async def test_quarter_correction_asks_part_before_replacing_existing_cart() -> None:
    services = FakeConversationServices()
    services.session.add_cart_item(cart_item_from_product(services.products["BROASTER_ENTERO"], 1))
    services.session.move_to(ConversationState.POST_ADD)
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Un cuarto perdon"))

    assert result["current_step"] == ConversationState.ASK_CHICKEN_PART
    assert "pierna o pechuga" in result["response_text"].lower()
...
@pytest.mark.asyncio
async def test_three_quarters_adds_one_unit_after_composition_selection() -> None:
    services = FakeConversationServices()
    services.session.selected_product_code = ProductCode("BROASTER_34")
    services.session.move_to(ConversationState.ASK_CHICKEN_PART)
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="2 pechugas 1 pierna"))

    assert result["current_step"] == ConversationState.POST_ADD
    assert [(item.product_code.value, item.quantity, item.product_name.value) for item in services.session.cart] == [
        ("BROASTER_34", 1, "3/4 Broasted - 2 pechugas y 1 pierna")
    ]
    assert "Cuantas unidades" not in result["response_text"]


@pytest.mark.asyncio
async def test_fragmented_bare_quarter_order_keeps_pending_context_and_saves_address() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    first = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Un cuarto"))
    second = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Pollo"))
    style = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Asado"))
    address = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Car 6# 46-07 lagos2"))
    choice_question = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Pechuga o pierna pernil"))
    thinking = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="🤔"))
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Pierna"))
    payment = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="En efectivo"))

    assert first["current_step"] == ConversationState.ASK_CHICKEN_STYLE
    assert "asado o broster" in first["response_text"].lower()
    assert second["current_step"] == ConversationState.ASK_CHICKEN_STYLE
    assert "asado o broster" in second["response_text"].lower()
    assert style["current_step"] == ConversationState.ASK_CHICKEN_PART
    assert "pierna o pechuga" in style["response_text"].lower()
    assert address["current_step"] == ConversationState.ASK_CHICKEN_PART
    assert services.session.customer_address == "Car 6# 46-07"
    assert services.session.customer_neighborhood == "Lagos 2"
    assert "Puede ser pechuga o pierna pernil" in choice_question["response_text"]
    assert thinking["current_step"] == ConversationState.ASK_CHICKEN_PART
    assert "pierna o pechuga" in thinking["response_text"].lower()
    assert added["current_step"] == ConversationState.POST_ADD
    assert [(item.product_code.value, item.product_name.value, item.quantity) for item in services.session.cart] == [
        ("ASADO_CUARTO", "1/4 Asado - Pierna", 1)
    ]
    assert "metodo de pago" in added["response_text"]
    assert payment["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "nombre completo, telefono" in payment["response_text"]
...
@pytest.mark.asyncio
async def test_half_asado_typo_with_inline_customer_data_adds_directly() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Buenas me vende medio pollo asdo\n"
                "Luis Gomez\n"
                "3174121205\n"
                "Cra 6 # 46-07 Lagos ll\n"
                "nequi"
            ),
        )
    )
    ...
    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "1 x 1/2 Asado" in result["response_text"]
    assert "¿Lo quieres asado o broster?" not in result["response_text"]
    assert [item.product_code.value for item in services.session.cart] == ["ASADO_MEDIO"]
    assert services.session.customer_name == "Luis Gomez"
    assert services.session.phone == "3174121205"
    assert services.session.customer_address == "Cra 6 # 46-07"
    assert services.session.customer_neighborhood == "Lagos 2"
...
@pytest.mark.asyncio
async def test_half_chicken_ask_quantity_adds_directly() -> None:
    services = FakeConversationServices()
    state = ConversationGraphState(
        chat_id=123,
        raw_text="quiero medio pollo asado",
        current_step=ConversationState.ASK_CHICKEN_PART,
        selected_product_code="ASADO_CUARTO",
        selected_product_name="1/2 Asado",
        selected_unit_price_cop=22300,
    )

    result = await nodes.ask_quantity(state, services)
...
@pytest.mark.asyncio
async def test_half_chicken_text_overrides_stale_pending_part_state() -> None:
    services = FakeConversationServices()
...
@pytest.mark.asyncio
async def test_real_multiline_checkout_order_keeps_joined_quarter_fraction_and_phone() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert [
        (item.product_code.value, item.product_name.value, item.quantity)
        for item in services.session.cart
    ] == [
        ("ASADO_CUARTO", "1/4 Asado - Pierna", 1),
        ("BROASTER_MEDIO", "1/2 Broasted", 1),
    ]
    assert services.session.customer_name == "Juliana"
    assert services.session.customer_phone == "3024459317"
    assert services.session.customer_address == "Calle 42 #4-84"
    assert services.session.customer_neighborhood == "Lagos 2"
    assert services.session.payment_method == "Efectivo"
    assert "me falta" not in result["response_text"].lower()
...
@pytest.mark.asyncio
async def test_ambiguous_half_chicken_with_polo_typo_asks_style_before_adding() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="medio polo"))
...
async def test_misspelled_three_quarters_with_perniles_adds_one_three_quarter_asado() -> None:
    ...
    assert result["customer"]["neighborhood"] == "Lagos 2"
    assert "nombre completo" in result["response_text"]
    assert "telefono" in result["response_text"]
    assert "metodo de pago" in result["response_text"]
...
async def test_pickup_whole_roasted_after_availability_saves_time_name_and_only_asks_phone() -> None:
    ...
    availability = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Aún tienes pollo asado?"))
    pickup = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Me apartas uno entero por favor, en 20 minutos paso a recogerlo",
        )
    )
    name = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="A nombre de Pedro Rivero por favor"))

    assert "Pollo asado disponible" in availability["response_text"]
    assert pickup["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert pickup["fulfillment_type"] == "PICKUP"
    assert "- 1 x 1 Asado Entero: $44500" in pickup["response_text"]
    assert "Nombre completo" in pickup["response_text"]
    assert "Telefono" in pickup["response_text"]
    assert "En cuanto tiempo pasa a recoger" not in pickup["response_text"]
    assert services.session.fulfillment_type == "PICKUP"
    assert services.session.observations and "20 minutos" in services.session.observations
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [("ASADO_ENTERO", 1)]

    assert name["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "Me falta esta informacion: telefono" in name["response_text"]
    assert "direccion" not in name["response_text"].lower()
    assert "barrio" not in name["response_text"].lower()
    assert services.session.customer_name == "Pedro Rivero"
    assert services.session.customer_address == "Recoge en local"
    assert services.session.customer_neighborhood == "No aplica"
...
@pytest.mark.asyncio
async def test_confirmed_order_followups_do_not_restart_menu_or_product_selection() -> None:
    services = FakeConversationServices()
    services.session.pending_order_json = {
        "kind": "last_confirmed_order",
        "observations": None,
        "items": [
            {"code": "BROASTER_CUARTO", "name": "1/4 Broasted - Pierna", "quantity": 1},
        ],
    }
    services.session.move_to(ConversationState.MAIN_MENU)
    graph = build_conversation_graph(services)
    ...
    time_note = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="12.30"))
    no_soup_note = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Sin sopa"))
    ...
    assert "tu orden ya está confirmada y en proceso" in time_note["response_text"]
    assert "Para las 12:30" in time_note["response_text"]
    assert "pierna o pechuga" not in time_note["response_text"].lower()
    assert "tu orden ya está confirmada y en proceso" in no_soup_note["response_text"]
    assert "Sin sopa" in no_soup_note["response_text"]
    assert "pierna o pechuga" not in no_soup_note["response_text"].lower()
    assert "Para las 12:30." in str(services.session.pending_order_json.get("observations"))
    assert "Sin sopa." in str(services.session.pending_order_json.get("observations"))
    assert "sopa va incluida" in soup["response_text"]
    assert "Adicionales" not in soup["response_text"]
    assert waiting["response_text"] == "Con mucho gusto, gracias a ti por elegirnos."
...
@pytest.mark.asyncio
async def test_pickup_receiver_name_without_cart_saves_data_without_opening_menu() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    assert result["current_step"] == ConversationState.NATURAL_ORDER
    assert result["fulfillment_type"] == "PICKUP"
    assert services.session.customer_name == "Luis Malagon"
    assert services.session.fulfillment_type == "PICKUP"
    assert "ya tengo tus datos" in result["response_text"].lower()
    assert "Bienvenid@ a Mac Chicken" not in result["response_text"]


@pytest.mark.asyncio
async def test_pickup_quarter_piece_without_style_asks_style_and_keeps_receiver_name() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="1/4 pechuga con sopa,recoge Luis Malagon")
    )
    ...
    assert added["current_step"] == ConversationState.POST_ADD
    assert "1 x 1/4 Asado - Pechuga" in added["response_text"]
    assert "Nombre completo" not in added["response_text"]
    assert "Telefono" in added["response_text"]
    assert "En cuanto tiempo pasa a recoger" in added["response_text"]
    assert services.session.customer_name == "Luis Malagon"
...
async def test_broaster_quarters_with_customer_data_and_bellavista_delivery_checkout() -> None:
    ...
    assert "1 x 3/4 Broasted - 2 piernas y 1 pechuga: $38200" in result["response_text"]
    ...
    assert [item.quantity for item in services.session.cart] == [1]
    ...
    assert "Total: $42200" in no_drink["response_text"]
    ...
    assert [(item.product_name, item.quantity) for item in services.synced_orders[0].items] == [
        ("3/4 Broasted - 2 piernas y 1 pechuga", 1),
    ]
    assert services.session.cart == []
...
@pytest.mark.asyncio
async def test_real_customer_add_manzana_25_button_does_not_become_address() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Tienen servidion"))
    first = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Un pollo a la Broaster porfa y una gaseosa litro manzana",
    ...
    assert "gaseosa Manzana solo la manejamos en presentacion 2.5 L" in first["response_text"]
    assert "1 x Gaseosa 2.5 L - Manzana" in added["response_text"]
    assert [item.product_code.value for item in services.session.cart] == [
        "BROASTER_ENTERO",
        "GASEOSA_25",
    ]
    assert services.session.cart[1].product_name == ProductName("Gaseosa 2.5 L - Manzana")
    assert services.session.customer_address is None
    assert services.session.customer_neighborhood == "el manantial"
    assert "Direccion: Manzana 2.5" not in checkout["response_text"]
...
@pytest.mark.asyncio
async def test_pending_whole_and_quarters_applies_todo_broaster_to_all_items() -> None:
    services = FakeConversationServices()
    services.products["ASADO_ENTERO"] = Product(
        code=ProductCode("ASADO_ENTERO"),
        name=ProductName("1 Asado Entero"),
    ...
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Buenas tardes"))
    style_question = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Me da porfavor 1 pollo y 2 cuartos de pollo pierna pernil",
        )
    )
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Todo broaste"))

    assert style_question["current_step"] == ConversationState.ASK_CHICKEN_STYLE
    assert "¿Lo quieres asado o broster?" in style_question["response_text"]
    assert added["current_step"] == ConversationState.POST_ADD
    assert "- 2 x 1/4 Broasted - Pierna: $27000" in added["response_text"]
    assert "- 1 x Broasted Entero: $51000" in added["response_text"]
    assert "Total acumulado: $78000" in added["response_text"]
    assert [(item.product_code.value, item.quantity, item.product_name.value) for item in services.session.cart] == [
        ("BROASTER_CUARTO", 2, "1/4 Broasted - Pierna"),
        ("BROASTER_ENTERO", 1, "Broasted Entero"),
    ]


@pytest.mark.asyncio
async def test_checkout_keeps_lagos_dos_barrio_and_ignores_payment_filler_note() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Buenas tardes"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Medio pollo a la broaster"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Patricia reyes"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Calle 42 5-69 lagos dos"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="En efectivo"))
    review = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Patricia reyes  calle 42 5-69 lagos dos 3013609702 pago en efectivo",
        )
    )

    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "📍 Direccion: calle 42 5-69" in review["response_text"]
    assert "🏘️ Barrio: lagos dos" in review["response_text"]
    assert "📝 Nota: Sin nota" in review["response_text"]
    assert "Domicilio: $2000" in review["response_text"]
    assert "🏘️ Barrio: En" not in review["response_text"]
    assert "📝 Nota: En" not in review["response_text"]
    assert services.session.customer_neighborhood == "lagos dos"
    assert services.session.observations is None
...
async def test_fragmented_lasagna_checkout_rebuilds_fields_and_accepts_cash_alias(
    ...
    monkeypatch.setattr(nodes, "_business_today", lambda: date(2026, 8, 2))
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
...
@pytest.mark.asyncio
async def test_fragmented_checkout_keeps_fields_when_cash_alias_arrives_later(monkeypatch) -> None:
    monkeypatch.setattr(nodes, "_business_today", lambda: date(2026, 8, 2))
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    review = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Contado"))

    assert partial["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "Me falta esta informacion: metodo de pago" in partial["response_text"]
    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "👤 Cliente: Italo Valero" in review["response_text"]
    assert "📍 Direccion: Cra28a 193-26" in review["response_text"]
    assert "🏘️ Barrio: Villa Piedra Del Sol" in review["response_text"]
...
@pytest.mark.asyncio
async def test_included_two_soups_availability_does_not_offer_paid_soup_addon() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Y al a broaster"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Y que trae el broaster"))
    result = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Tienen las 2 sopas disponibles")
    )
    followup = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Por qué esas si las necesito para mis 2 hijos")
    )

    assert "sí tenemos sopa disponible" in result["response_text"]
    assert "incluye 2 sopas" in result["response_text"]
    assert "Sopa Adicional" not in result["response_text"]
    assert "¿Deseas ordenarla" not in result["response_text"]
    assert "sí tenemos sopa disponible" in followup["response_text"]
    assert "Bienvenid@ a Mac Chicken" not in followup["response_text"]
    assert services.session.cart == []


@pytest.mark.asyncio
async def test_emphatic_included_soup_availability_question_answers_soup_state() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Si tienen disponible las sopas")
    )
...
@pytest.mark.asyncio
async def test_included_two_soups_availability_uses_unavailable_soup_state() -> None:
    services = FakeConversationServices()
    ...
    assert "no tenemos sopa disponible" in result["response_text"]
    assert "Sopa Adicional" not in result["response_text"]
    assert services.session.cart == []


@pytest.mark.asyncio
async def test_confirmed_order_dispatch_followup_and_pointer_do_not_open_service_or_cart() -> None:
    services = FakeConversationServices()
    services.session.pending_order_json = {
        "kind": "last_confirmed_order",
        "observations": None,
        "items": [
            {"code": "ASADO_MEDIO", "name": "1/2 Asado", "quantity": 1},
        ],
    }
    services.session.move_to(ConversationState.MAIN_MENU)
    graph = build_conversation_graph(services)

    dispatch = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Ya saldría el domicilio")
    )
    pointer = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="?"))
    delayed = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="No ha llegado mi domicilio")
    )

    assert "despachar tu pedido" in dispatch["response_text"]
    assert "mejor experiencia" in dispatch["response_text"]
    assert "servicio a domicilio" not in dispatch["response_text"].lower()
    assert "despachar tu pedido" in pointer["response_text"]
    assert "Tu orden esta vacia" not in pointer["response_text"]
    assert "despachar tu pedido" in delayed["response_text"]
    assert "servicio a domicilio" not in delayed["response_text"].lower()
...
@pytest.mark.asyncio
async def test_soup_question_after_cart_repeats_missing_checkout_fields() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Oye para pedir medio pollo asado y medio a la broster",
        )
    )
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Viene con sopa?"))
...
@pytest.mark.asyncio
async def test_confirmed_order_eta_and_labeled_villa_piedra_checkout_regression() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Viene con sopa?"))
    review = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Jonatan yesid Laython jaimes\n"
                "3108824016\n"
                "Calle195#28-09\n"
                "Barrio: villa piedra del sol\n"
                "Efectivo y completo"
            ),
        )
    )
    confirmed = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="si"))
    eta = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Más o menos en cuanto llega")
    )

    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "🏘️ Barrio: villa piedra del sol" in review["response_text"]
    assert "🏘️ Barrio: y completo" not in review["response_text"]
    assert "Orden confirmada" in confirmed["response_text"]
    assert eta["response_text"] == (
        "Para pedidos a domicilio, el tiempo aproximado de entrega es de 30 minutos."
    )
    assert "Cuantas unidades" not in eta["response_text"]
...
@pytest.mark.asyncio
async def test_labeled_checkout_note_with_split_pechugas_adds_quarters_not_whole_broaster() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Nombre: Jesús Duarte Ayala\n"
                "Direccion y barrio: Mediterrane Royal Anillo vial\n"
                "Telefono: 3104630846\n"
                "Metodo de pago: Nequi\n"
                "Nota o especificacion (opcional): me puede vender por favor dos pechugas, una asada y una broaster"
            ),
        )
    )
    ...
    assert "1 x 1/4 Asado - Pechuga" in result["response_text"]
    assert "1 x 1/4 Broasted - Pechuga" in result["response_text"]
    assert "Broasted Entero" not in result["response_text"]
    assert "Me falta esta informacion: direccion" in result["response_text"]
    assert services.session.customer_neighborhood == "Mediterrane Royal Anillo vial"
    assert not services.session.customer_address
    assert services.session.payment_method == "Nequi"
...
def _add_asado_entero_to_fake_catalog(services: FakeConversationServices) -> None:
    services.products["ASADO_ENTERO"] = Product(
        code=ProductCode("ASADO_ENTERO"),
        name=ProductName("1 Asado Entero"),
        category=ProductCategory.POLLO_ASADO,
...
@pytest.mark.asyncio
async def test_order_with_residential_complex_reference_keeps_it_as_neighborhood_and_reasks_missing_data() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)
    ...
    added = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="1 pollo asado\nConjunto Valmonti casa 68",
        )
    )
    delivery = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Y el domicilio ?"))

    assert added["current_step"] == ConversationState.POST_ADD
    assert "- 1 x 1 Asado Entero: $44500" in added["response_text"]
    assert "nombre completo" in added["response_text"]
    assert "telefono" in added["response_text"]
    assert "direccion" in added["response_text"]
    assert "barrio" not in added["response_text"].lower()
    assert "metodo de pago" in added["response_text"]
    assert services.session.customer_neighborhood == "Conjunto Valmonti casa 68"
    assert services.session.customer_address is None

    assert delivery["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "Me falta esta informacion" in delivery["response_text"]
    assert "direccion" in delivery["response_text"]
    assert "barrio" not in delivery["response_text"].lower()
    assert "servicio a domicilio" not in delivery["response_text"].lower()


@pytest.mark.asyncio
async def test_labeled_address_with_residential_reference_still_requires_exact_address() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="1 pollo asado"))
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Nombre: Laura Perez\n"
                "Telefono: 3101234567\n"
                "Direccion: Conjunto Valmonti casa 68\n"
                "Metodo de pago: Efectivo"
            ),
        )
    )
...
@pytest.mark.asyncio
async def test_real_failed_chat_adicionales_opens_menu_while_phone_is_missing() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="adicionales"))

    assert result["current_step"] == ConversationState.SELECT_ADICIONAL
    assert "Adicionales" in result["response_text"]
    assert "Me falta esta informacion" not in result["response_text"]
    assert services.session.cart


@pytest.mark.asyncio
async def test_customer_data_sent_from_addons_menu_is_saved_until_order_arrives() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Buenos días"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="adicionales"))

    data = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Juan Carlos Mantilla Sanguino\n"
                "CRA 31 #155-24 asovilagos\n"
                "3177594646\n"
                "Transferencia\n"
                "Con buen aji"
            ),
    ...
    assert data["current_step"] == ConversationState.NATURAL_ORDER
    assert "La cuenta de Nequi" not in data["response_text"]
    assert "ya tengo tus datos" in data["response_text"].lower()
    assert "Que deseas ordenar" in data["response_text"]
    assert services.session.customer_name == "Juan Carlos Mantilla Sanguino"
    assert services.session.customer_phone == "3177594646"
    assert services.session.payment_method == "Transferencia Bancolombia"
    assert "buen aji" in (services.session.observations or "").lower()

    review = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Un pollo asado"))

    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "Datos recibidos. Revisa tu orden" in review["response_text"]
    assert "- 1 x 1 Asado Entero: $44500" in review["response_text"]
    assert "👤 Cliente: Juan Carlos Mantilla Sanguino" in review["response_text"]
    assert "📞 Telefono: 3177594646" in review["response_text"]
    assert "💳 Pago: Transferencia Bancolombia" in review["response_text"]
    assert "¿Confirmas tu orden?" in review["response_text"]


@pytest.mark.asyncio
async def test_real_failed_chat_waiting_followups_do_not_reask_missing_checkout_data() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    polite = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="¡Vale! Quedo atenta"))
    waiting = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Es que ya van 40 minuticos 🥲"))

    assert "Me falta esta informacion" not in polite["response_text"]
    assert "Me falta esta informacion" not in waiting["response_text"]
    assert "pedido" in waiting["response_text"].lower() or "orden" in waiting["response_text"].lower()
    assert services.session.cart


@pytest.mark.asyncio
async def test_real_failed_chat_barrio_label_is_not_left_inside_address() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Buenos días. Dame por favor lo siguiente:\n"
                "- 1 lasaña \n"
                "- ⁠1 cuarto de pollo broaster pierna pernil \n"
                "- ⁠1 pollo asado completo\n\n"
                "Dirección: Calle 39 #5-125, barrio Lagos 2\n"
                "Pago en efectivo, ¿cuánto es el total?\n\n"
                "*Que esté acá a las 12:30pm, por favor, antes no*"
            ),
        )
    )

    assert ("BROASTER_CUARTO", 1) in [
        (item.product_code.value, item.quantity) for item in services.session.cart
    ]
    assert ("ASADO_ENTERO", 1) in [
        (item.product_code.value, item.quantity) for item in services.session.cart
    ]
    assert services.session.customer_address == "Calle 39 #5-125"
    assert services.session.customer_neighborhood == "Lagos 2"
    assert "barrio barrio" not in result["response_text"].lower()


@pytest.mark.asyncio
async def test_real_failed_chat_split_whole_chicken_adds_half_asado_and_half_broaster() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
...
@pytest.mark.asyncio
async def test_real_failed_chat_pickup_mixed_order_keeps_asado_style_and_pickup_data() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    assert [(item.product_code.value, item.product_name.value, item.quantity) for item in services.session.cart] == [
        ("ASADO_MEDIO", "1/2 Asado", 1),
        ("BROASTER_CUARTO", "1/4 Broasted - Pechuga", 1),
    ]
    assert services.session.fulfillment_type == "PICKUP"
    assert services.session.customer_name == "Santiago"
    assert services.session.observations == "Recoger a la 1 pm"
...
@pytest.mark.asyncio
async def test_real_failed_chat_three_broaster_quarters_with_inline_data_reaches_review() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert [(item.product_name.value, item.quantity) for item in services.session.cart] == [
        ("3/4 Broasted - 2 piernas y 1 pechuga", 1),
    ]
    assert "Total: $42200" in result["response_text"]
...
@pytest.mark.asyncio
async def test_real_checkout_review_keeps_clean_phone_and_answers_time_total_before_confirming() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    review = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="wendy 3022873946 efectivo")
    )

    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "📞 Telefono: 3022873946" in review["response_text"]
    assert "wendy 3022873946" not in review["response_text"]
    assert "Total: $49800" in review["response_text"]

    status = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Cuanto se demora? Y cuanto es?")
    )
...
@pytest.mark.asyncio
async def test_real_failed_chat_included_soup_and_sauce_note_does_not_add_paid_soup() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
...
@pytest.mark.asyncio
async def test_real_full_asado_with_respective_soup_does_not_add_paid_soup_or_duplicate_order() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)
    ...
    review = await graph.ainvoke(
        ConversationGraphState(
            ...
            raw_text=(
                "Alejandra Quintero\n"
                "CRA 42#88-75 altos de Caracoli floridablanca\n"
                "3058279223\n"
                "Tranferencia\n"
                "1 pollo asado con su respectiva sopa"
            ),
        )
    )
    soup_question = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="El pollo asado viene con su sopa ?")
    )
    ...
    assert "1 x 1 Asado Entero" in review["response_text"]
    assert "Sopa Adicional" not in review["response_text"]
    assert "Pago: Transferencia Bancolombia" in review["response_text"]
    assert "metodo de pago" not in review["response_text"].lower()

    assert soup_question["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "incluye 2 sopas sin costo" in soup_question["response_text"].lower()
    assert "Añadido a tu orden" not in soup_question["response_text"]
    assert "Sopa Adicional" not in soup_question["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("ASADO_ENTERO", 1)
    ]
...
@pytest.mark.asyncio
async def test_non_exact_market_local_address_is_saved_as_note_and_blocks_nequi_account() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    order = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Buenas porfavor me vende medio pollo asado con sopa para el local de jugos la colmena lagos ll",
        )
    )
    data = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "nombre  completo: Stefany Camacho\n"
                "telefono 3184502272\n"
                "direccion plaza de mercado la colmena lagos ll local de jugos\n"
                "barrio: lagos ll\n"
                "metodo de pago: nequi"
            ),
        )
    )
    account = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Porfavor me envía el nequi para pagar")
    )

    assert "- 1 x 1/2 Asado: $22300" in order["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("ASADO_MEDIO", 1)
    ]
    assert services.session.customer_name == "Stefany Camacho"
    assert services.session.phone == "3184502272"
    assert services.session.payment_method == "Nequi"
    assert services.session.customer_neighborhood == "Lagos 2"
    assert services.session.customer_address is None
    assert "plaza de mercado la colmena local de jugos" in (services.session.observations or "")
    assert "lagos ll" not in nodes.normalize_text(services.session.observations)
    assert "telefono" not in data["response_text"].lower()
    assert "metodo de pago" not in data["response_text"].lower()
    assert "direccion completa con calle/carrera, numero y barrio" in data["response_text"].lower()
    assert "3182705144" not in account["response_text"]
    assert "direccion completa con calle/carrera, numero y barrio" in account["response_text"].lower()


@pytest.mark.asyncio
async def test_quarter_pernil_before_style_is_used_and_non_exact_address_does_not_reset() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Audio recibido"))
    ask_style = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Buena tarde x favor me puedes enviar un cuarto de pollo q sea pernil "
                "para la plaza para el local de jugos"
            ),
        )
    )
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Asado"))
    data = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Maria Márquez\n3209247968\nPlaza la colmena de lagos 2\nEfectivo",
        )
    )

    assert ask_style["current_step"] == ConversationState.ASK_CHICKEN_STYLE
    assert "¿Lo quieres asado o broster?" in ask_style["response_text"]
    assert added["current_step"] == ConversationState.POST_ADD
    assert "- 1 x 1/4 Asado - Pierna: $11800" in added["response_text"]
    assert "¿Lo quieres en pierna o pechuga?" not in added["response_text"]
    assert services.session.cart[0].product_code == ProductCode("ASADO_CUARTO")
    assert services.session.cart[0].product_name == ProductName("1/4 Asado - Pierna")
    assert services.session.customer_name == "Maria Márquez"
    assert services.session.phone == "3209247968"
    assert services.session.customer_address is None
    assert services.session.customer_neighborhood == "Lagos 2"
    assert services.session.payment_method == "Efectivo"
    assert "plaza la colmena" in (services.session.observations or "").lower()
    assert data["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "direccion completa con calle/carrera, numero y barrio" in data["response_text"].lower()
    assert "Bienvenid@" not in data["response_text"]


@pytest.mark.asyncio
@pytest.mark.parametrize("raw_neighborhood", ["lagos ll", "lagos LL", "lagos ii", "lagos II", "lagos dos", "lagos 2"])
async def test_lagos_two_variants_are_normalized(raw_neighborhood: str) -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="1/2 asado"))
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=f"Maria Gomez\n3101234567\nCalle 47 #3-53\n{raw_neighborhood}\nEfectivo",
        )
    )
...
@pytest.mark.asyncio
async def test_unavailable_specific_drink_question_offers_similar_drink_without_touching_chicken_order() -> None:
    services = FakeConversationServices()
    services.products["JUGO_HIT_LITRO"] = Product(
        code=ProductCode("JUGO_HIT_LITRO"),
        name=ProductName("Jugo Hit Litro"),
        category=ProductCategory.BEBIDAS,
        price=MoneyCOP(6000),
    )
    graph = build_conversation_graph(services)
    ...
    order = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Me vende por favor medio pollo a la broaster")
    )
    offer = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Tiene mr tea litro?"))

    assert "- 1 x 1/2 Broasted: $25500" in order["response_text"]
    assert "no contamos con mr tea litro" in offer["response_text"].lower()
    assert "Jugo Hit Litro" in offer["response_text"]
    assert "Puedes tocar Agregar Jugo Hit Litro" in offer["response_text"]
    assert "Bebidas" in offer["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("BROASTER_MEDIO", 1)
    ]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] != [
    ...
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Jugo Hit Litro"))

    assert "- 1 x Jugo Hit Litro: $6000" in added["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("BROASTER_MEDIO", 1),
        ("JUGO_HIT_LITRO", 1),
    ]
...
@pytest.mark.asyncio
async def test_existing_specific_drink_question_adds_drink_directly() -> None:
    services = FakeConversationServices()
    services.products["JUGO_HIT_LITRO"] = Product(
        code=ProductCode("JUGO_HIT_LITRO"),
        name=ProductName("Jugo Hit Litro"),
    ...
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Tiene Jugo Hit Litro?"))

    assert "- 1 x Jugo Hit Litro: $6000" in result["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("JUGO_HIT_LITRO", 1)
    ]
...
@pytest.mark.asyncio
async def test_half_combo_availability_question_offers_order_and_order_button_adds_both_halves() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    question = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Tiene 1/2 pollo a la Broaster y 1/2 asado?")
    )

    assert question["current_step"] == ConversationState.ASK_HALF_COMBO
    assert "1/2 pollo asado" in question["response_text"]
    assert "1/2 pollo broaster" in question["response_text"]
    assert "¿Deseas ordenarlos ahora" in question["response_text"]
    assert services.session.cart == []

    ordered = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="ordenar"))

    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("ASADO_MEDIO", 1),
        ("BROASTER_MEDIO", 1),
    ]
    assert "1 x 1/2 Asado" in ordered["response_text"]
    assert "1 x 1/2 Broasted" in ordered["response_text"]


@pytest.mark.asyncio
async def test_soup_followup_after_half_combo_query_asks_included_or_additional_without_charging() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Y 1 sopa"))

    assert "sopa incluida" in result["response_text"].lower()
    assert "sopa adicional" in result["response_text"].lower()
    assert services.session.cart == []


@pytest.mark.asyncio
async def test_labeled_customer_template_ignores_cedula_and_adds_later_maduro_to_review() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="ordenar"))
    review = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "datos📝\n\n"
                "Número de cédula: 1.098.646.551\n"
                "Nombres y Apellidos: Yuly Milena Silva Rendón\n"
                "Dirección y barrio: Calle 196C #27D-21 Barrio el Recreo\n"
                "Teléfono de contacto:3186544801\n"
                "Correo electronico: Yulymilenasilva@hotmail.com\n"
    ...
    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "👤 Cliente: Yuly Milena Silva Rendón" in review["response_text"]
    assert "📞 Telefono: 3186544801" in review["response_text"]
    assert "📍 Direccion: Calle 196C #27D-21" in review["response_text"]
    assert "🏘️ Barrio: el Recreo" in review["response_text"]
    assert "💳 Pago: Datafono" in review["response_text"]
    assert "cédula" not in review["response_text"].lower()
    assert "correo" not in review["response_text"].lower()

    updated = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Y adicional 1 maduro"))

    assert updated["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "Maduro con Queso" in updated["response_text"]
    assert "👤 Cliente: Yuly Milena Silva Rendón" in updated["response_text"]
    assert "¿Confirmas tu orden?" in updated["response_text"]


@pytest.mark.asyncio
async def test_confirmed_order_sauce_note_and_fabio_holder_followups_are_acknowledged() -> None:
    services = FakeConversationServices()
    _add_asado_entero_to_fake_catalog(services)
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="1 asado"))
    await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="wendy\n3022873946\ncra28a#195-33\nel manantial\nefectivo",
    ...
    note = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Por favor bastante tártara y piña Si tienes")
    )
    holder = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Fab per?"))

    assert "Nota actualizada" in note["response_text"]
    assert "tártara" in note["response_text"]
    assert "piña" in note["response_text"]
    assert "Fabio Leonardo Perez" in holder["response_text"]


@pytest.mark.asyncio
async def test_fragmented_request_to_talk_with_fabio_is_handed_to_advisor() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Buenos\nDias"))
    result = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Puedo\nHablar\nCon\nFabio")
    )

    assert result["response_text"] == "Te voy a remitir con un asesor para que te ayude."
    assert "Fabio Leonardo Perez" not in result["response_text"]


@pytest.mark.asyncio
async def test_mediterrane_royal_address_and_anillo_vial_continuation_are_collected() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="medio pollo asado"))
    missing_address = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Nombre: Jesús Duarte Ayala\n"
                "Telefono: 3104630846\n"
                "Barrio: Mediterrane Royal\n"
                "Metodo de pago: Nequi"
            ),
        )
    )

    assert missing_address["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "direccion" in missing_address["response_text"].lower()

    still_missing = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="si por favor"))

    assert still_missing["current_step"] == ConversationState.ASK_CUSTOMER_DATA

    phone_repeat = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="ya antes me han enviado con mi número de telefono es 3104630846",
        )
    )

    assert phone_repeat["current_step"] == ConversationState.ASK_CUSTOMER_DATA

    review = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="la dirección es carrera 12 n 200-14 torre 2 apt 303 conjunto mediterrane royal",
        )
    )

    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "📍 Direccion: carrera 12 n 200-14 torre 2 apt 303" in review["response_text"]
    assert "🏘️ barrio: mediterrane royal" in review["response_text"].lower()

    updated = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="anillo vial"))

    assert updated["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "📍 Direccion: carrera 12 n 200-14 torre 2 apt 303" in updated["response_text"]
    assert "🏘️ barrio: mediterrane royal anillo vial" in updated["response_text"].lower()


@pytest.mark.asyncio
async def test_lasagna_availability_offer_can_be_ordered_by_followup_and_completed_with_data() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Buena tarde"))
    availability = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Tiene lasaña?"))
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Me regalas una a domicilio"))
    partial = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Bucarica sector 4 bloque 8-1"))
    review = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Lizeth Pérez\nBucarica sector 4 bloque 8-1\n3184837970\nEfectivo, completo",
        )
    )

    assert "Lasagna Mixta está disponible" in availability["response_text"]
    assert "¿Deseas ordenarla ahora" in availability["response_text"]
    assert "1 x Lasagna Mixta" in added["response_text"]
    assert "$20000" in added["response_text"]
    assert "Me falta esta informacion" in partial["response_text"]
    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "- 1 x Lasagna Mixta: $20000" in review["response_text"]
    assert "👤 Cliente: Lizeth Pérez" in review["response_text"]
    assert "📞 Telefono: 3184837970" in review["response_text"]
    assert "🏘️ Barrio: Bucarica" in review["response_text"]
    assert "💳 Pago: Efectivo" in review["response_text"]
    assert "¿Confirmas tu orden?" in review["response_text"]


@pytest.mark.asyncio
async def test_unavailable_quarter_piece_plain_yes_accepts_recommended_alternative() -> None:
    services = FakeConversationServices()
    unavailable_code = "BROASTER_CUARTO_PECHUGA"
    services.unavailable_variant_codes.add(unavailable_code)
    services.recommended_variant_alternatives[unavailable_code] = (
        "BROASTER_CUARTO",
        "Pierna",
    ...
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Hola buenas tardes"))
    await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Me regala un cuarto de pollo broster")
    )
    unavailable = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Pechuga"))
    accepted = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="si"))
    review = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "Walter Andrés\n"
                "3212567152\n"
                "Calle 44#5-79 lagos 2\n"
                "Pago en efectivo"
            ),
        )
    )

    assert "1/4 Broasted - Pechuga no esta disponible" in unavailable["response_text"]
    assert "1/4 Broaster - Pierna" in unavailable["response_text"]
    assert accepted["current_step"] == ConversationState.POST_ADD
    assert "1 x 1/4 Broasted - Pierna" in accepted["response_text"]
    assert "productos" not in accepted["response_text"].lower()
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("BROASTER_CUARTO", 1),
    ]
    assert review["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "- 1 x 1/4 Broasted - Pierna: $13500" in review["response_text"]
    assert "Tu orden esta vacia" not in review["response_text"]


@pytest.mark.asyncio
async def test_direct_unavailable_quarter_piece_yes_adds_one_without_quantity_prompt() -> None:
    services = FakeConversationServices()
    unavailable_code = "BROASTER_CUARTO_PECHUGA"
    services.unavailable_variant_codes.add(unavailable_code)
    services.recommended_variant_alternatives[unavailable_code] = (
    ...
    unavailable = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Quiero un cuarto de pollo broster pechuga")
    )
    accepted = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="si"))

    assert unavailable["current_step"] == ConversationState.ASK_STOCK_ALTERNATIVE
    assert "1/4 Broasted - Pechuga no esta disponible" in unavailable["response_text"]
    assert "1/4 Broaster - Pierna" in unavailable["response_text"]
    assert accepted["current_step"] == ConversationState.POST_ADD
    assert "1 x 1/4 Broasted - Pierna: $13500" in accepted["response_text"]
    assert "Cuantas unidades" not in accepted["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("BROASTER_CUARTO", 1),
    ]
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("question", "product_code", "product_name", "category", "price"),
    [
        ("Hay maduro con queso?", "MADURO_QUESO", "Maduro con Queso", ProductCategory.ESPECIALES, 9500),
        ("Hay coca cola 1.5?", "COCA_COLA_15", "Coca-Cola 1.5 L", ProductCategory.BEBIDAS, 8500),
        ("Tienen papa francesa?", "PAPA_FRANCESA", "Papa Francesa", ProductCategory.ADICIONALES, 8200),
    ],
)
async def test_concrete_product_availability_offer_works_for_any_category(
    question: str,
    product_code: str,
    product_name: str,
    ...
    services = FakeConversationServices()
    services.products[product_code] = Product(
        code=ProductCode(product_code),
        name=ProductName(product_name),
        category=category,
        price=MoneyCOP(price),
    )
    graph = build_conversation_graph(services)

    availability = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text=question))
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="ordenar"))

    assert f"{product_name} está disponible" in availability["response_text"]
    assert "¿Deseas ordenarla ahora" in availability["response_text"]
    assert product_name in added["response_text"]
    assert f"${price}" in added["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [(product_code, 1)]


@pytest.mark.asyncio
async def test_inline_labeled_customer_data_can_include_order_inside_note_field() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            ...
            raw_text=(
                "Nombre: Yuri Duque. "
                "Dirección: calle 195 # 28-15 barrio Villa Piedra del Sol. "
                "Celular: 3204776411. "
                "Pago: efectivo. "
                "Nota: medio pollo asado y medio pollo a la broaster, 1 maduro con queso"
            ),
        )
    )
    ...
    assert "- 1 x 1/2 Asado: $22300" in result["response_text"]
    assert "- 1 x 1/2 Broasted: $25500" in result["response_text"]
    assert "- 1 x Maduro con Queso: $9500" in result["response_text"]
    assert "👤 Cliente: Yuri Duque" in result["response_text"]
    assert "📞 Telefono: 3204776411" in result["response_text"]
    assert "📍 Direccion: calle 195 # 28-15" in result["response_text"]
    assert "🏘️ Barrio: Villa Piedra del Sol" in result["response_text"]
    assert "💳 Pago: Efectivo" in result["response_text"]
    assert "Si, recibimos Efectivo" not in result["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("ASADO_MEDIO", 1),
...
async def test_disordered_pre_order_part_payment_is_applied_to_later_quarter_order() -> None:
    ...
    account = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Me regalas el número si me hace el favor")
    )

    assert fragment["current_step"] == ConversationState.NATURAL_ORDER
    assert services.session.payment_method == "Nequi"
    assert "Presa solicitada: Pierna." in (services.session.observations or "")
    assert order["current_step"] == ConversationState.ASK_CUSTOMER_DATA
    assert "- 1 x 1/4 Broasted - Pierna: $13500" in order["response_text"]
    assert "Domicilio: $2000" in order["response_text"]
    assert "nombre completo" in order["response_text"]
    assert "telefono" in order["response_text"]
    assert "barrio" not in order["response_text"].lower()
    assert services.session.customer_address == "sector 5 bloque 6-3 apto 401"
    assert services.session.customer_neighborhood == "bucarica"
    assert account["response_text"] == "La cuenta de Nequi es 3182705144 a nombre de Fabio Leonardo Perez."
    assert services.session.customer_name is None


@pytest.mark.asyncio
async def test_previous_day_session_is_cleared_before_new_greeting() -> None:
    services = FakeConversationServices()
    services.session.add_cart_item(cart_item_from_product(services.products["ASADO_CUARTO"], 1))
    services.session.customer_name = "Luis Malagon"
    services.session.phone = "3130000000"
    services.session.address = "Recoge en local"
    services.session.neighborhood = "No aplica"
    services.session.payment_method = "Efectivo"
    services.session.observations = "Recoger en 10 minutos"
    services.session.fulfillment_type = "PICKUP"
    services.session.move_to(ConversationState.ASK_CUSTOMER_DATA)
    services.session.last_seen_at = datetime.combine(
        date(2026, 7, 31),
        datetime.min.time(),
    ...
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Hola"))

    assert result["current_step"] == ConversationState.MAIN_MENU
    assert "Bienvenid" in result["response_text"]
    assert "Me falta esta informacion" not in result["response_text"]
    assert services.session.cart == []
    assert services.session.customer_name is None
    assert services.session.payment_method is None
...
@pytest.mark.asyncio
async def test_pickup_order_uses_recoge_name_payment_and_pickup_time() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    pickup = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Recoge Luis Malagon"))
    payment = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Efectivo"))
    time = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Paso en 10 min se puede?"))

    assert pickup["fulfillment_type"] == "PICKUP"
    assert services.session.customer_name == "Luis Malagon"
    assert services.session.address == "Recoge en local"
    assert services.session.neighborhood == "No aplica"
    assert services.session.payment_method == "Efectivo"
    assert "telefono" in time["response_text"]
    assert "en cuanto tiempo pasa a recoger" not in time["response_text"]
    assert "10 min" in (services.session.observations or "")


@pytest.mark.asyncio
async def test_typo_brostr_is_understood_as_half_broasted() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Medio brostr"))
...
@pytest.mark.asyncio
async def test_pickup_time_alone_is_saved_even_when_product_is_still_missing() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Paso a recoger para Cesar Mantilla"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="3174121205"))
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="15 minutos"))

    assert "Recoge en 15 minutos" in (services.session.observations or "")
    assert services.session.customer_name == "Cesar Mantilla"
    assert "Que deseas ordenar" in result["response_text"]


@pytest.mark.asyncio
async def test_hit_personal_does_not_ask_flavor_and_old_flavor_wait_accepts_new_drink() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    hit = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="2.Jugo Hoy Persona 3000"))
    assert "sabor" not in hit["response_text"].lower()

    services.session.cart = []
    services.session.selected_product_code = ProductCode("JUGO_HIT_PERSONAL")
    services.session.selected_product_name = ProductName("Jugos Hit personal")
    services.session.move_to(ConversationState.ASK_PRODUCT_VARIANT)
    coca = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="6.Coca-colq personal 400 ml"))

    assert "Coca-Cola personal 400 ml" in coca["response_text"]
    assert all(item.product_code.value != "JUGO_HIT_PERSONAL" for item in services.session.cart)
    assert any(item.product_code.value == "PERSONAL_400" for item in services.session.cart)


@pytest.mark.asyncio
async def test_stock_alternative_saves_customer_data_then_asks_again_for_acceptance() -> None:
    services = FakeConversationServices()
    unavailable_code = "BROASTER_CUARTO_PECHUGA"
    services.unavailable_variant_codes.add(unavailable_code)
    services.recommended_variant_alternatives[unavailable_code] = (
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Walter Andres\n3212567152\nCalle 44#5-79 lagos 2\nPago en efectivo",
        )
    )

    assert services.session.customer_name == "Walter Andres"
    assert services.session.phone == "3212567152"
    assert services.session.payment_method == "Efectivo"
    assert services.session.current_step == ConversationState.ASK_STOCK_ALTERNATIVE
    assert "1/4 Broasted - Pierna" in result["response_text"]
    assert "Deseas seguir" in result["response_text"]


@pytest.mark.asyncio
async def test_nequi_and_cash_typos_are_payment_methods_while_missing_data() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Medio broster"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Luis Perez"))
    nequi = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="nequi"))
    assert services.session.payment_method == "Nequi"
    assert "metodo de pago" not in nequi["response_text"].lower()

    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    await graph.ainvoke(ConversationGraphState(chat_id=124, raw_text="Medio broster"))
    await graph.ainvoke(ConversationGraphState(chat_id=124, raw_text="Luis Perez"))
    typo_cash = await graph.ainvoke(ConversationGraphState(chat_id=124, raw_text="efectibo"))
    assert services.session.payment_method == "Efectivo"
    assert "metodo de pago" not in typo_cash["response_text"].lower()


@pytest.mark.asyncio
async def test_only_that_is_not_saved_as_name_and_next_name_is_kept() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Medio broster"))
    only = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Solamente eso"))
    name = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Luis Perez"))

    assert services.session.customer_name == "Luis Perez"
    assert services.session.customer_neighborhood is None
    assert "nombre completo" in only["response_text"].lower()
    assert "nombre completo" not in name["response_text"].lower()


@pytest.mark.asyncio
async def test_inline_template_without_colons_extracts_data_and_keeps_non_exact_address_as_note() -> None:
    services = FakeConversationServices()
    services.products["ASADO_ENTERO"] = Product(
        code=ProductCode("ASADO_ENTERO"),
        name=ProductName("1 Asado Entero"),
    ...
    result = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text=(
                "nombre completo: Stefany Camacho telefono 3184502272 "
                "direccion plaza de mercado la colmena lagos ll local de jugos "
                "barrio: lagos ll metodo de pago: nequi"
            ),
        )
    )
    ...
    assert "plaza de mercado la colmena" in (services.session.observations or "")
    assert "direccion completa con calle/carrera, numero y barrio" in result["response_text"].lower()


@pytest.mark.asyncio
async def test_pickup_with_saved_data_shows_review_after_later_product() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Paso a recoger para Cesar Mantilla"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="3174121205"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="15 minutos"))
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Medio brostr"))

    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "Revisa tu orden para recoger" in result["response_text"]
    assert "👤 Cliente: Cesar Mantilla" in result["response_text"]
    assert "📞 Telefono: 3174121205" in result["response_text"]
    assert "📝 Nota: Recoge en 15 minutos" in result["response_text"]


@pytest.mark.asyncio
async def test_non_exact_reference_note_is_kept_after_exact_address_arrives() -> None:
    services = FakeConversationServices()
    services.products["ASADO_ENTERO"] = Product(
        code=ProductCode("ASADO_ENTERO"),
        name=ProductName("1 Asado Entero"),
    ...
    await graph.ainvoke(
        ConversationGraphState(
            ...
            raw_text=(
                "Stefany Camacho\n"
                "3184502272\n"
                "plaza de mercado la colmena lagos ll local de jugos\n"
                "Lagos ll\n"
                "Nequi"
            ),
        )
    )
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Cra 6 # 46-07 Lagos 2"))

    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "📍 Direccion: Cra 6 # 46-07" in result["response_text"]
    assert "🏘️ Barrio: Lagos 2" in result["response_text"]
    assert "📝 Nota: plaza de mercado la colmena local de jugos" in result["response_text"]
    assert "📝 Nota: Lagos ll" not in result["response_text"]


@pytest.mark.asyncio
async def test_lasagna_and_maduro_are_available_without_calendar_block() -> None:
    services = FakeConversationServices()
    services.products["LASAGNA_MIXTA"] = Product(
        code=ProductCode("LASAGNA_MIXTA"),
        name=ProductName("Lasagna Mixta"),
    ...
    services.products["MADURO_QUESO"] = Product(
        code=ProductCode("MADURO_QUESO"),
        name=ProductName("Maduro con Queso"),
        category=ProductCategory.ESPECIALES,
        price=MoneyCOP(9500),
    ...
    result = await graph.ainvoke(
        ConversationGraphState(chat_id=123, raw_text="Me regala dos lasagna y un maduro con queso")
    )

    assert "solo esta disponible fines de semana" not in result["response_text"]
    assert "- 2 x Lasagna Mixta: $40000" in result["response_text"]
    assert "- 1 x Maduro con Queso: $9500" in result["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("LASAGNA_MIXTA", 2),
        ("MADURO_QUESO", 1),
...
@pytest.mark.asyncio
async def test_stock_alternative_acceptance_with_saved_data_shows_checkout_review() -> None:
    services = FakeConversationServices()
    unavailable_code = "BROASTER_CUARTO_PECHUGA"
    services.unavailable_variant_codes.add(unavailable_code)
    services.recommended_variant_alternatives[unavailable_code] = (
    ...
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="si"))

    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "Revisa tu orden" in result["response_text"]
    assert "- 1 x 1/4 Broasted - Pierna: $13500" in result["response_text"]
    assert "👤 Cliente: Walter Andres" in result["response_text"]


@pytest.mark.asyncio
async def test_fragmented_fabio_request_hands_off_to_advisor() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)

    for text in ("Buenos", "Dias", "Puedo", "Hablar", "Con"):
        await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text=text))
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Fabio"))

    assert result["response_text"] == "Te voy a remitir con un asesor para que te ayude."


@pytest.mark.asyncio
async def test_takeaway_with_address_is_delivery_and_quarter_restatement_replaces_cart() -> None:
    services = FakeConversationServices()
    services.products["BROASTER_ENTERO"] = Product(
        code=ProductCode("BROASTER_ENTERO"),
        name=ProductName("Broasted Entero"),
        category=ProductCategory.POLLO_BROASTER,
        price=MoneyCOP(51000),
    ...
    first = await graph.ainvoke(
        ConversationGraphState(
            chat_id=123,
            raw_text="Un pollo Broaster para llevar\nCalle 44 #3_79 lagos II",
        )
    )
    ask_part = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Un cuarto perdon"))
    added = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="2"))

    assert first["fulfillment_type"] == "DELIVERY"
    assert "direccion" not in first["response_text"].lower()
    assert "Lo quieres en pierna o pechuga" in ask_part["response_text"]
    assert [(item.product_code.value, item.quantity) for item in services.session.cart] == [
        ("BROASTER_CUARTO", 1),
    ]
    assert "1/4 Broasted - Pechuga" in added["response_text"]
    assert services.session.customer_address == "Calle 44 #3_79"
    assert services.session.customer_neighborhood == "Lagos 2"
    assert services.session.fulfillment_type == "DELIVERY"
...
@pytest.mark.asyncio
async def test_mediterrane_exact_address_does_not_keep_label_residue_note() -> None:
    services = FakeConversationServices()
    graph = build_conversation_graph(services)
    ...
    await graph.ainvoke(
        ConversationGraphState(
            ...
            raw_text=(
                ...
                "Telefono: 3104630846\n"
                "Metodo de pago: Nequi\n"
                "Nota: dos pechugas, una asada y una broaster"
            ),
        )
    )
    result = await graph.ainvoke(
    ...
    assert result["current_step"] == ConversationState.CHECKOUT_REVIEW
    assert "📍 Direccion: carrera 12 n 200-14 torre 2 apt 303" in result["response_text"]
    assert "🏘️ Barrio: mediterrane royal" in result["response_text"]
    assert "Direccion y" not in result["response_text"]


@pytest.mark.asyncio
async def test_delivery_price_complaint_is_not_saved_as_customer_name() -> None:
    services = FakeConversationServices()
    services.products["ASADO_ENTERO"] = Product(
        code=ProductCode("ASADO_ENTERO"),
        name=ProductName("1 Asado Entero"),
    ...
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Con datáfono, calle 45No 6-78lagos dos"))
    await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="3185531074"))
    result = await graph.ainvoke(ConversationGraphState(chat_id=123, raw_text="Acá a lagos ochomil?"))
